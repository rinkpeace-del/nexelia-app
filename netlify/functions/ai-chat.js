exports.handler = async (event) => {
  ...
  try {
    const { question, context, prompt } = JSON.parse(event.body);

    if (!question) {
      return { statusCode: 400, body: JSON.stringify({ error: "question is required" }) };
    }

    const finalPrompt = prompt || `中学生向け英語学習サイト NEXELIA を使っています。\n現在の画面: ${context || 'NEXELIA'}\n\n質問:\n${question}\n\n日本語でやさしく解説して、最後に確認クイズを1問出してください。`;
    ...
    const answer = data.choices?.[0]?.message?.content?.trim() || "回答を生成できませんでした。もう一度試してください。";
