...
(function () {
  ...
  function addStyles() {
    ...
    style.textContent = [
      ...
      '.retry-body{flex:1;overflow:hidden;display:flex;flex-direction:column;max-width:680px;width:100%;margin:0 auto;padding:36px 24px 28px;}',
      '.retry-slide{display:none;flex-direction:column;flex:1;min-height:0;overflow-y:auto;animation:retryFadeUp .3s ease both;}',
  ...
  function buildRetryOverlay(wrongIndices) {
    ...
    wrongIndices.forEach(function (origIdx, retryIdx) {
      ...
      // 問題番号を「問題 N / M」に書き換える
      var qLabel = clone.querySelector('.q-label');
      if (qLabel) qLabel.textContent = '問題 ' + (retryIdx + 1) + ' / ' + total;
